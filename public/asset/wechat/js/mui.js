...
(function($, window) {
	...
	var Slider = $.Scroll.extend({
		...
		_init: function() {
			...
			//add by jclarry@qq.com
			if(this.options.isScroll){
				
				var items=this.wrapper.querySelectorAll('.mui-control-item');
				var wid=this.wrapper.clientWidth;
				for (var i = 0, len = items.length; i < len; i++) {
					items[i].style.width=wid/this.options.pageSize+"px";
				}
			}
			if (this.scroller) {
		...
		//API
		setTranslate: function(x, y) {
			//update by jclarry@qq.com
			this._super(x, y);
			var wid=this.wrapperWidth
			//获取滚动对象
			var indicator = this.wrapper.querySelector(SELECTOR_SLIDER_INDICATOR);
			var progressBar = this.progressBar;
			if (progressBar) {
				//debugger
				var offset=(-x * (this.progressBarWidth / this.wrapperWidth));
				var pagesize=this.options.pageSize;
				var centerPage=pagesize-2-this.options.scrollIndex;//Math.floor(pagesize/2)+1;
				if(this.options.isScroll){
					
					var num=this.currentPage.pageX;//当前页的下标
					if(num==this.itemLength-1&&(this.options.scrollIndex==-1)){//特殊处理当为scrollIndex=-1时执行
						indicator.style.webkitTransform = this._getTranslateStr(-wid/pagesize*(num-centerPage), 0);
						this.progressBarStyle.webkitTransform = this._getTranslateStr(offset-wid/pagesize*(num-centerPage), 0);
					}else if(num==this.itemLength-1){
						this.progressBarStyle.webkitTransform = this._getTranslateStr(offset-wid/pagesize*(num-centerPage-1-this.options.scrollIndex), 0);
					}else if(num>=this.itemLength-1-this.options.scrollIndex){
						var i=this.itemLength-num-2;
						indicator.style.webkitTransform = this._getTranslateStr(-wid/pagesize*(num+i-centerPage-this.options.scrollIndex), 0);
						this.progressBarStyle.webkitTransform = this._getTranslateStr(offset-wid/pagesize*(num-centerPage-this.options.scrollIndex+i), 0);
					}else if(num>centerPage&&offset==this.progressBarWidth*num&&num<this.itemLength-1-this.options.scrollIndex){//当前页大于设置的中心项坐标时
						this.isfirst=1;
						indicator.style.webkitTransform = this._getTranslateStr(-wid/pagesize*(num-centerPage), 0);
						this.progressBarStyle.webkitTransform = this._getTranslateStr(offset-wid/pagesize*(num-centerPage), 0);
					}else if(num==centerPage){
						if(this.isfirst==0){
							indicator.style.webkitTransform = this._getTranslateStr(-wid/pagesize*(num-centerPage), 0);
							this.progressBarStyle.webkitTransform = this._getTranslateStr(offset-wid/pagesize*(num-centerPage), 0);
						}else{
							indicator.style.webkitTransform = this._getTranslateStr(-wid/pagesize*(num-centerPage+1), 0);
							this.progressBarStyle.webkitTransform = this._getTranslateStr(offset-wid/pagesize*(num-centerPage+1), 0);
						}
					}else if(num==centerPage-1&&this.isfirst==1){
						indicator.style.webkitTransform = this._getTranslateStr(-wid/pagesize*(num-centerPage+1), 0);
						this.progressBarStyle.webkitTransform = this._getTranslateStr(offset-wid/pagesize*(num-centerPage+1), 0);
	
					}else{
						this.progressBarStyle.webkitTransform = this._getTranslateStr(offset, 0);
					}
				}else{
					this.progressBarStyle.webkitTransform = this._getTranslateStr(offset, 0);
				}
				//设置滚动
				//indicator.style.webkitTransform = this._getTranslateStr(-offset, 0);
			}
		},
		resetPosition: function(time) {
			time = time || 0;
