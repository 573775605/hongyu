...
@section('content')
    <div class="ibox float-e-margins">
        <ul class="nav nav-tabs">
            <li class="active"><a href="javascript:void(0)" style="font-size: 15px;"><i class="fa"></i>广告列表</a></li>
            <a href="{{url('admin/banner/add')}}">
                <button type="button" class="btn btn-w-m btn-primary pull-right" style="margin-bottom: 0px; line-height:33px;">添加广告</button>
            </a>
        </ul>
                        ...
                        <div class="example">
                            <table data-toggle="table" data-height="100%" data-mobile-responsive="true" data-card-view="false">
                                ...
                                @foreach($rows->getItems() as $v)
                                    <tr>
                                        <td>{{$v->title}}</td>
                                        <td><img src="{{$v->img->url or ''}}" alt="" width="250" height="100"></td>
                                        <td>{{$v->url}}</td>
                                        <td>{{$v->status?'已启用':'未启用'}}</td>
                                        <td>{{$v->create_time}}</td>
                                        <td>{{$v->sort}}</td>
                                        <td>
                                            <a href="{{url('admin/banner/edit').'/'.$v->id}}" class="mod btn btn-info">编辑</a>
