...
@section('content')
    <div class="ibox float-e-margins">
        <ul class="nav nav-tabs">
            <li class="active"><a href="javascript:void(0)" style="font-size: 15px;"><i class="fa"></i>商品货源配置</a></li>
            <a href="{{url('admin/goods-source/add')}}">
                <button type="button" class="btn btn-w-m btn-primary pull-right" style="margin-bottom: 0px; line-height:33px;">添加货源</button>
            </a>
        </ul>
                            ...
                            <table data-toggle="table" data-height="100%" data-mobile-responsive="true" data-card-view="false">
                                ...
                                @foreach($rows->getItems() as $v)
                                    <tr>
                                        ...
                                        <td>{{$v->total_grade}}星</td>
                                        <td>{{$v->sort}}</td>
                                        <td>
                                            <a href="{{url('admin/goods-source/edit').'/'.$v->id}}" class="mod btn btn-info">编辑</a>
    ...
    <script>
        function remove(id) {
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        $.post("{{url('admin/goods-source/remove')}}/" + id,
                            {_token: '{{csrf_token()}}'},
