...
@section('content')
    <div class="ibox float-e-margins">
        <ul class="nav nav-tabs">
            <li class="active"><a href="javascript:void(0)" style="font-size: 15px;"><i class="fa"></i>商品单位配置</a></li>
            <a href="{{url('admin/goods-unit/add')}}">
                            ...
                            <table data-toggle="table" data-height="100%" data-mobile-responsive="true" data-card-view="false">
                                ...
                                @foreach($rows->getItems() as $v)
                                    <tr>
                                        ...
                                        <td>{{$v->sort}}</td>
                                        <td>
                                            <a href="{{url('admin/goods-unit/edit').'/'.$v->id}}" class="mod btn btn-info">编辑</a>
    ...
    <script>
        function remove(id) {
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        $.post("{{url('admin/goods-unit/remove')}}/" + id,
                            {_token: '{{csrf_token()}}'},
